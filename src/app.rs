...
impl App {
    pub fn new() -> Self {
        ...
        let mut app = Self {
            ...
            substrate: CellularAutomata::new(seed ^ 0xC011, 160, 90),
    ...
    fn evolve_attraction_matrix(&mut self, intensity: f32) {
        ...
        for a in 0..TRIBE_COUNT {
            for b in 0..TRIBE_COUNT {
                ...
                let mutation_drift = if a != b {
                    ((a as f32 * 1.37 + b as f32 * 2.11 + self.age as f32 * 0.007).cos())
                        * mutation_pressure
                        * intensity
                        * 0.004
                } else {
                ...
                self.rules[a][b] =
                    (value + stabilizer + corridor_split + mutation_drift + chaos_drift)
                        .clamp(-1.0, 1.0);
            }
        }
    ...
    pub fn reset_particles(&mut self) {
        ...
        self.substrate = CellularAutomata::new(self.seed ^ self.age ^ 0xC011, 160, 90);
    ...
    fn shift_environment(&mut self) {
        ...
        self.environment = if pathfinder_bias > 0.55 && roll > 40 {
            Environment::Drift
        } else {
            match roll {
                0..=44 => Environment::Calm,
                45..=58 => Environment::Bloom,
                59..=68 => Environment::Hunger,
                69..=84 => Environment::Drift,
                _ => Environment::Storm,
            }
        };

        self.push_event(&format!(
            "environment shifted to {}",
